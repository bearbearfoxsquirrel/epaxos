...
var catchupBatchSize *int = flag.Int("catchupbatch", -1, "How many instances will replicas respond with in catch up requests after recovery")
...
func main() {
	...
	if *maxInitBackoff == 0 {
		*maxInitBackoff = int(float64(*minBackoff) * 1.1)
	}
	...
	if *doEpaxos {
		...
		rep := mencius.NewReplica(replicaId, nodeList, *thrifty, *exec, *lread, *dreply, *durable, *maxfailures, *storageParentDir, *emulatedSS, emulatedWriteTime, int32(*deadTime))
		rpc.Register(rep)
	} else if *doGpaxos {
