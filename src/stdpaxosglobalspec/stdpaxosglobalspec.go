...
//	// is chosen or preemped (conf curBal no longer relevant
//	// 1 recieve value
//	// 2 no val received
...
//					if count < maxExtraInstances && r.isMoreCommitsToComeAfter(i) {
//						pc.MoreToCome = 1
...
//		return
//	}
//	if pbk.curBal.GreaterThan(preply.Cur) || pbk.status != PREPARING {
//		dlog.Printf("Message in late \n")
//		return
//	} else if r.proposerCheckAndHandlePreempt(preply.Instance, preply.Cur, PROMISE) {
//		dlog.Printf("Another active proposer using ballot %d.%d greater than mine\n", preply.Cur)
//		r.acceptorPrepareOnConfBal(preply.Instance, preply.Cur)
//		minSafe := inst.abk.curBal
//		r.checkAndHandleOldPreempted(preply.Cur, minSafe, inst.abk.vConfBal, inst.abk.cmds, preply.Instance)
...
//		// here is where we can add fast learning bit - also add acceptance by curBal's owner
//		if r.fastLearn {
...
//	gap := r.crtInstance - r.executedUpTo
//	if gap == r.maxOpenInstances {
//		dlog.Printf("not opening up new instances")
//	}
//	for i := r.maxOpenInstances - gap; i > 0; i-- {
//		r.beginNextInstance()
//	}
//}
//
